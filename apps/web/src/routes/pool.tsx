...
/// "Are you in this week?" status card. Three states:
///   - disconnected → invite to open in playground
///   - connected, eligible → 🎟 badge
///   - connected, not eligible → CTA "Pay any service to enter"
function ViewerStatusCard() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const ok = await readViewerInThisWeek(viewer);
      ...
      <Card className="border-emerald-500/40 bg-emerald-500/5">
        <CardContent className="flex items-center gap-3 py-4">
          ...
          <div className="text-sm leading-snug">
            ...
            <p className="text-[11px] text-[var(--color-muted)]">
              One paying buyer per week wins the pot. Good luck Sunday.
            </p>
          </div>
        </CardContent>
      </Card>
    ...
    <Card className="border-amber-500/30 bg-amber-500/5">
      <CardContent className="flex items-center gap-3 py-4">
        ...
        <div className="flex-1 text-sm leading-snug">
          ...
          <p className="text-[11px] text-[var(--color-muted)]">
            Pay any service with a community share to enter — automatic.
          </p>
        </div>
        <Link
          to="/services"
...
/// Stack of avatars for who's in this week's draw. Cap at 12 visible +
/// "+N more" pill so a popular week doesn't run off the card.
function EntriesThisWeekCard() {
  const [entries, setEntries] = useState<Address[] | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const list = await readWeekEntries(state.currentWeekIndex);
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {entries.length === 0
            ? "No one's in the draw yet this week."
            : entries.length === 1
              ? '1 buyer is queued for Sunday.'
              : `${entries.length} buyers are queued for Sunday.`}
        </CardDescription>
      </CardHeader>
      {entries.length > 0 && (
        <CardContent>
    ...
    <Card>
      ...
      <CardContent className="space-y-3 text-sm leading-relaxed">
        ...
        <p>
          ...
          and you're entered for the week's draw.
        </p>
        <p>
          <strong>3.</strong> Sunday at 18:00 UTC, a random eligible
          buyer is selected on-chain via{' '}
          <code className="font-mono text-[11px]">block.prevrandao</code>.
        </p>
        <p>
          <strong>4.</strong> The winner claims their TKP, then can
          redeem it back to CRC at any time via the pool's policy.
        </p>
        <p className="text-[11px] text-[var(--color-muted)]">
          No protocol fee. No yield. Just redistribution between humans
          who pay each other.
        </p>
      </CardContent>
    </Card>
